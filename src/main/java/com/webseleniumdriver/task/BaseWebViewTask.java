...
public abstract class BaseWebViewTask {
    ...
    public void updateListView(String message) {
        ...
        MessageListView item = new MessageListView(content, "RED");
    ...
    protected final void scrollBy(int x, int y) {
        if (!isValidTaskInRunning()) return;
        try {
            print("scrollBy " + x + "/" + y);
            String scrip = String.format("window.scrollBy(%o,%o)", x, y);
